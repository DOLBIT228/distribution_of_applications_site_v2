# 📥 Розподіл заявок Bitrix24 (Web + VPS-ready)

Застосунок для авто-розподілу заявок у Bitrix24 між менеджерами.

Тепер у проєкті є **окремий веб-інтерфейс на Flask** (`backend_service.py`), до якого можна підключатися по URL, а сам бекенд розміщувати на VPS.
...
- `distribution_history.db` створюється автоматично поруч із кодом застосунку.
